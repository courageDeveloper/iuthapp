...
@Component({
  selector: 'app-view-revenue-counterproduct',
  templateUrl: './view-revenue-counterproduct.component.html',
  styleUrls: ['./view-revenue-counterproduct.component.css']
})
export class ViewRevenueCounterProductComponent implements OnInit {
  ...
  loadCounterProducts() {
    this.pouchService.getCounterProducts().then(items => {
      items = items.filter(data => data.branch == 'IUTH(Okada)');
  ...
  public export(): void {
    ...
    this.pouchService.getCounterProducts().then(items => {
      ...
      this.saveAsExcelFile(this.excelBuffer, 'IUTH Product(Counter Product)');
    });
  }
