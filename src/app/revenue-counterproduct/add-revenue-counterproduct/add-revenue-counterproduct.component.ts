...
@Component({
  selector: 'app-add-revenue-counterproduct',
  templateUrl: './add-revenue-counterproduct.component.html',
  styleUrls: ['./add-revenue-counterproduct.component.css']
})
export class AddRevenueCounterProductComponent implements OnInit {
  title;
  ...
  constructor(private router: Router, public dialogRef: MatDialogRef<AddRevenueCounterProductComponent>, @Inject(MAT_DIALOG_DATA) public data: any, public toastr: ToastrService, public _DomSanitizer: DomSanitizer, public pouchService: PouchService, private formBuilder: FormBuilder) {
  ...
  sendNotification(branch) {
    var notification = {
      ...
      url: this.router.url,
      viewed: false,
      message: `${this.counterProduct.productname} selling price has been added at Revenue`,
    ...
    this.pouchService.getStaffs().then(staffs => {
      ...
      staffs = staffs.filter(data => data.department == 'Revenue' || data.department == this.counterProduct.sourcedepartment || data.department == 'Account' || data.department == 'Audit');
