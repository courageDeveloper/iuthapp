...
export class ViewGopdPharmacyServiceComponent implements OnInit {
  ...
  ngOnInit() {
    this.pouchService.userPermission().then(result => {
      if (result.department == 'GOPD Pharmacy') {
  ...
  reloadRenderServices() {
    this.pouchService.getRenderServices().then(items => {
      ...
      this.pouchService.paginateByDepartmentRemoveItem('renderservice', this.pouchService.paginationId, 'GOPD Pharmacy').then(paginatedata => {
