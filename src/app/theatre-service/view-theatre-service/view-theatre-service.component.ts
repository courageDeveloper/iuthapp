...
import { AddTheatreServiceComponent } from '../add-theatre-service/add-theatre-service.component';
...
@Component({
  selector: 'app-view-theatre-service',
  templateUrl: './view-theatre-service.component.html',
  styleUrls: ['./view-theatre-service.component.css']
})
export class ViewTheatreServiceComponent implements OnInit {
  ...
  reloadRenderServices() {
    this.pouchService.getRenderServices().then(items => {
      items = items.filter(data => data.branch == 'IUTH(Okada)' && data.department == "Theatre");
      ...
      this.pouchService.paginateByDepartmentRemoveItem('renderservice', this.pouchService.paginationId, 'Theatre').then(paginatedata => {
  ...
  loadRenderServices() {
    this.pouchService.getRenderServices().then(items => {
      ...
      this.pouchService.paginateByDepartment2('renderservice', this.pouchService.paginationId, 'Theatre').then(paginatedata => {
        this.paginatedRenderServices = paginatedata;
  ...
  previous() {
    ...
    this.pouchService.paginateByDepartmentPrev2('renderservice', this.pouchService.paginationId, 'Theatre').then(paginatedata => {
      this.paginatedRenderServices = paginatedata;
  ...
  goToStart() {
    ...
    this.pouchService.paginateByDepartmentStart('renderservice', this.pouchService.paginationId, 'Theatre').then(paginatedata => {
      this.paginatedRenderServices = paginatedata;
  ...
  editRenderService(renderService) {
    let dialogRef = this.dialog.open(AddTheatreServiceComponent, {
      width: '500px',
      data: {
  ...
  checkViewStatus() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      ...
      this.pouchService.getDepartments().then(departments => {
        departments = departments.filter(data => data.name == "Theatre" && data.branch == staff.branch);
  ...
  public export(): void {
    ...
    this.pouchService.getRenderServices().then(items => {
      ...
      this.saveAsExcelFile(this.excelBuffer, 'IUTH Theatre Services');
    });
  }
