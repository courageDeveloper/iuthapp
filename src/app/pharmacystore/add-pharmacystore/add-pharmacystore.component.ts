...
@Component({
  selector: 'app-add-pharmacystore',
  templateUrl: './add-pharmacystore.component.html',
  styleUrls: ['./add-pharmacystore.component.css']
})
export class AddPharmacyStoreComponent implements OnInit {
  title;
  ...
  constructor(private router: Router, public dialogRef: MatDialogRef<AddPharmacyStoreComponent>, @Inject(MAT_DIALOG_DATA) public data: any, public toastr: ToastrService, public _DomSanitizer: DomSanitizer, public pouchService: PouchService, private formBuilder: FormBuilder) {
  ...
  ngOnInit() {
    ...
    this.pouchService.getProductcategorys().then(items => {
      items = items.filter(data => data.department == 'Pharmacy Store' && data.branch == 'IUTH(Okada)')
      this.products = items;
    });
  ...
  sendNotification(branch) {
    var notification = {
      ...
      url: this.router.url,
      viewed: false,
      message: `${this.product.unitstock} ${this.product.productname} has been supplied to Pharmacy Store for a price of ${this.product.stockvalue} `,
