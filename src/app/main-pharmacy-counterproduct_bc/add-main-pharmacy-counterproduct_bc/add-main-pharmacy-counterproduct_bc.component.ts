...
@Component({
  selector: 'app-add-main-pharmacy-counterproduct_bc',
  templateUrl: './add-main-pharmacy-counterproduct_bc.component.html',
  styleUrls: ['./add-main-pharmacy-counterproduct_bc.component.css']
})
export class AddMainPharmacyCounterProductBcComponent implements OnInit {
  title;
  ...
  constructor(private router: Router, public dialogRef: MatDialogRef<AddMainPharmacyCounterProductBcComponent>, @Inject(MAT_DIALOG_DATA) public data: any, public toastr: ToastrService, public _DomSanitizer: DomSanitizer, public pouchService: PouchService, private formBuilder: FormBuilder) {
  ...
  sendNotification(branch) {
    ...
    this.pouchService.getStaffs().then(staffs => {
      ...
      staffs = staffs.filter(data => data.department == 'Main Pharmacy' || data.department == this.counterProduct.sourcedepartment || data.department == 'Account' || data.department == 'Audit');
