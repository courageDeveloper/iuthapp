...
@Component({
  selector: 'app-add-laboratory-counterproduct',
  templateUrl: './add-laboratory-counterproduct.component.html',
  styleUrls: ['./add-laboratory-counterproduct.component.css']
})
export class AddLaboratoryCounterProductComponent implements OnInit {
  title;
  ...
  constructor(private router: Router, public dialogRef: MatDialogRef<AddLaboratoryCounterProductComponent>, @Inject(MAT_DIALOG_DATA) public data: any, public toastr: ToastrService, public _DomSanitizer: DomSanitizer, public pouchService: PouchService, private formBuilder: FormBuilder) {
  ...
  sendNotification(branch) {
    var notification = {
      ...
      url: this.router.url,
      viewed: false,
      message: `${this.counterProduct.productname} selling price has been added at Laboratory`,
    ...
    this.pouchService.getStaffs().then(staffs => {
      ...
      staffs = staffs.filter(data => data.department == 'Laboratory' || data.department == this.counterProduct.sourcedepartment || data.department == 'Account' || data.department == 'Audit');
