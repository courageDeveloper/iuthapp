...
@Component({
  selector: 'app-view-laboratory-counterproduct',
  templateUrl: './view-laboratory-counterproduct.component.html',
  styleUrls: ['./view-laboratory-counterproduct.component.css']
})
export class ViewLaboratoryCounterProductComponent implements OnInit {
  ...
  filterByMonth() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      this.pouchService.getCounterProducts().then(data => {
        ...
        this.pouchService.paginateByDepartment2('counterproduct', this.pouchService.paginationId, "Laboratory", undefined, undefined, undefined, undefined, 0).then(paginatedata => {
  ...
  loadDispatchedProducts() {
    this.pouchService.getDispatchedProducts().then(items => {
      items = items.filter(data => data.branch == 'IUTH(Okada)' && data.dispatchdepartment == "Laboratory");
      this.dispatchedProducts = items;
  ...
  reloadCounterProducts() {
    this.pouchService.getCounterProducts().then(items => {
      items = items.filter(data => data.branch == 'IUTH(Okada)' && data.department == "Laboratory" && data.totalsubitem > 0);
  ...
  loadCounterProducts() {
    this.pouchService.getCounterProducts().then(items => {
      ...
      this.pouchService.paginateByDepartment2('counterproduct', this.pouchService.paginationId, 'Laboratory', undefined, undefined, undefined, undefined, 0).then(paginatedata => {
        this.paginatedCounterProducts = paginatedata;
        console.log(this.paginatedCounterProducts);
  ...
  previous() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      ...
      this.pouchService.paginateByDepartmentPrev2('counterproduct', this.pouchService.paginationId, "Laboratory", undefined, undefined, undefined, undefined, 0).then(paginatedata => {
        this.paginatedCounterProducts = paginatedata;
  ...
  goToStart() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      ...
      this.pouchService.paginateByDepartmentStart('counterproduct', this.pouchService.paginationId, "Laboratory", undefined, undefined, undefined, undefined, 0).then(paginatedata => {
        this.paginatedCounterProducts = paginatedata;
  ...
  checkedExpired() {
    this.pouchService.getCounterProducts().then(items => {
      items = items.filter(data => data.branch == 'IUTH(Okada)' && data.department == "Laboratory");
      this.counterProducts = items;
  ...
  editCounterProduct(counterproduct) {
    let dialogRef = this.dialog.open(AddLaboratoryCounterProductComponent, {
      height: '500px',
      width: '500px',
      data: {
  ...
  sendRefundNotification(counterproduct) {
    ...
    this.pouchService.getStaffs().then(staffs => {
      ...
      staffs = staffs.filter(data => data.department == 'Laboratory' || data.department == counterproduct.sourcedepartment || data.department == 'Account' || data.department == 'Audit');
  ...
  checkViewStatus() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      ...
      this.pouchService.getDepartments().then(departments => {
        departments = departments.filter(data => data.name == "Laboratory" && data.branch == staff.branch);
  ...
  public export(): void {
    ...
    this.pouchService.getCounterProducts().then(items => {
      ...
      this.saveAsExcelFile(this.excelBuffer, 'IUTH Product(Laboratory Counter Product)');
    });
  }
