...
export class EvacuateformComponent implements OnInit {
  ...
  constructor(public pouchService: PouchService, private router: Router, private dialog: MatDialog, public dialogRef: MatDialogRef<EvacuateformComponent>,
  ...
  checkError() {
    if (this.dateFrom == undefined || this.timeFrom == undefined || this.dateTo == undefined || this.timeTo == undefined) {
  ...
  evacuateSales() {
    ...
    var totalMilliseconds = millisecondsHour + millisecondsMinute + dateFromMilliseconds;
    ...
    var totalMillisecondsTo = millisecondsHourTo + millisecondsMinuteTo + dateToMilliseconds;
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      this.pouchService.getSales().then(sales => {
        ...
        let p = sales.map(async sale => {
              ...
              newSaleDate = new Date(sale.date).toLocaleString("en-US", { timeZone: "GMT" });
              ...
              if (newDateFrom <= saleDateMillisecond && newDateTo >= saleDateMillisecond) {
  ...
  sendNotification(branch) {
    ...
    this.pouchService.getStaffs().then(staffs => {
      ...
      staffs = staffs.filter(data => data.department == 'Revenue' || data.department == 'Account' || data.department == 'Audit');
