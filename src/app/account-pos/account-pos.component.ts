...
export class AccountPosComponent implements OnInit {
  ...
  loadAllIndividualSales() {
    this.pouchService.getStaff(this.localStorageItem).then(staff => {
      this.pouchService.getIndividualSales().then(allIndividualSales => {
        this.allIndividualSales = allIndividualSales.filter(data => data.branch == 'Benin Centre');

        var currentAllIndividualSales = allIndividualSales.filter(data => data.branch == 'Benin Centre');
  ...
  checkReferenceNo() {
    ...
    this.pouchService.getSales().then(sales => {
      sales.forEach(sale => {
        if (sale.referencenumber.toLowerCase().trim() === this.referenceNumber.toLowerCase().trim() && sale.iscomplete == false) {
