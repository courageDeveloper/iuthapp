...
@Component({
  selector: 'app-view-dispatch-centralstore',
  templateUrl: './view-dispatch-centralstore.component.html',
  styleUrls: ['./view-dispatch-centralstore.component.css']
})
export class ViewDispatchCentralStoreComponent implements OnInit {
  ...
  filterByDate() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      this.pouchService.getDispatchedProducts().then(data => {
        ...
        this.dispatchedProducts = this.dispatchedProducts.filter(data => data.sourcedepartment == 'Central Store' && data.branch == 'IUTH(Okada)');
  ...
  public export(): void {
    ...
    this.pouchService.getDispatchedProducts().then(items => {
      items = items.filter(data => data.branch == 'IUTH(Okada)' && data.sourcedepartment == "Central Store");
      ...
      this.saveAsExcelFile(this.excelBuffer, 'IUTH Dispatched Product(Central Store)');
    });
  }
