...
@Component({
  selector: 'app-add-centralstore',
  templateUrl: './add-centralstore.component.html',
  styleUrls: ['./add-centralstore.component.css']
})
export class AddCentralStoreComponent implements OnInit {
  title;
  ...
  constructor(private router: Router, public dialogRef: MatDialogRef<AddCentralStoreComponent>, @Inject(MAT_DIALOG_DATA) public data: any, public toastr: ToastrService, public _DomSanitizer: DomSanitizer, public pouchService: PouchService, private formBuilder: FormBuilder) {
    ...
    if (this.data.action == 'add') {
      ...
      this.title = 'Edit';
      data.centralstore.datesupplied = new Date(data.centralstore.datesupplied);
      data.centralstore.expiryDate = new Date(data.centralstore.expiryDate);
      this.product = data.centralstore;
    }
  }
  ...
  ngOnInit() {
    ...
    this.pouchService.getProductcategorys().then(items => {
      items = items.filter(data => data.department == 'Central Store' && data.branch == 'IUTH(Okada)')
      this.products = items;
    });
  ...
  loadVendors() {
    this.pouchService.getVendors().then(vendors => {
      vendors = vendors.filter(data => data.branch == 'IUTH(Okada)');
