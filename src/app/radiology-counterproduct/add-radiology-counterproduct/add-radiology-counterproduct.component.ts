...
@Component({
  selector: 'app-add-radiology-counterproduct',
  templateUrl: './add-radiology-counterproduct.component.html',
  styleUrls: ['./add-radiology-counterproduct.component.css']
})
export class AddRadiologyCounterProductComponent implements OnInit {
  title;
  ...
  constructor(private router: Router, public dialogRef: MatDialogRef<AddRadiologyCounterProductComponent>, @Inject(MAT_DIALOG_DATA) public data: any, public toastr: ToastrService, public _DomSanitizer: DomSanitizer, public pouchService: PouchService, private formBuilder: FormBuilder) {
  ...
  sendNotification(branch) {
    var notification = {
      ...
      url: this.router.url,
      viewed: false,
      message: `${this.counterProduct.productname} selling price has been added at Radiology`,
    ...
    this.pouchService.getStaffs().then(staffs => {
      ...
      staffs = staffs.filter(data => data.department == 'Radiology' || data.department == this.counterProduct.sourcedepartment || data.department == 'Account' || data.department == 'Audit');
