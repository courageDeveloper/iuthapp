...
export class MainPharmacyPosComponent implements OnInit {
  ...
  loadCounterProducts() {
    this.pouchService.getCounterProducts().then(counterProducts => {
      counterProducts = counterProducts.filter(data => data.branch == 'IUTH(Okada)' && data.department == "Main Pharmacy" && data.isexpired == false);
  ...
  loadServices() {
    this.pouchService.getRenderServices().then(renderservices => {
      renderservices = renderservices.filter(data => data.branch == 'IUTH(Okada)' && data.department == "Main Pharmacy");
  ...
  viewPharmacyCounterProducts() {
    this.router.navigate(['view-main-pharmacy-counterproduct']);
  }
  ...
  checkout() {
    ...
    this.pouchService.getStaff(localStorageItem).then(item => {
      ...
      if (this.sales.balance > 0) {
        ...
        /*  if (this.sales.amount == this.totalAmountPaid) {
           this.sales.amount -= this.sales.balance;
         }
 
         if (this.sales.amount != this.totalAmountPaid) {
           this.sales.amount -= originalTotalAmountPaid;
         } */
      }
