...
@Component({
  selector: 'app-view-general-service',
  templateUrl: './view-general-service.component.html',
  styleUrls: ['./view-general-service.component.css']
})
export class ViewGeneralServiceComponent implements OnInit {
  ...
  reloadRenderServices() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      this.pouchService.getRenderServices().then(items => {
        items = items.filter(data => data.branch == staff.branch && data.department == "General");
        ...
        this.pouchService.paginateByDepartmentRemoveItem('renderservice', this.pouchService.paginationId, 'General').then(paginatedata => {
  ...
  loadRenderServices() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      this.pouchService.getRenderServices().then(items => {
        ...
        this.pouchService.paginateByDepartment2('renderservice', this.pouchService.paginationId, 'General').then(paginatedata => {
          this.paginatedRenderServices = paginatedata;
  ...
  previous() {
    ...
    this.pouchService.paginateByDepartmentPrev2('renderservice', this.pouchService.paginationId, 'General').then(paginatedata => {
      this.paginatedRenderServices = paginatedata;
  ...
  goToStart() {
    ...
    this.pouchService.paginateByDepartmentStart('renderservice', this.pouchService.paginationId, 'General').then(paginatedata => {
      this.paginatedRenderServices = paginatedata;
  ...
  editRenderService(renderService) {
    let dialogRef = this.dialog.open(AddGeneralServiceComponent, {
      width: '500px',
      data: {
  ...
  checkViewStatus() {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      ...
      this.pouchService.getDepartments().then(departments => {
        //departments = departments.filter(data => data.name == "General" && data.branch == staff.branch);
  ...
  public export(): void {
    ...
    this.pouchService.getStaff(localStorageItem).then(staff => {
      this.pouchService.getRenderServices().then(items => {
        ...
        this.saveAsExcelFile(this.excelBuffer, 'IUTH General Services');
      });
    });
  }
