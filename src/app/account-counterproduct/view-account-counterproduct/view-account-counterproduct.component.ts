...
@Component({
  ...
  templateUrl: './view-account-counterproduct.component.html',
  styleUrls: ['./view-account-counterproduct.component.css']
})
export class ViewAccountCounterProductComponent implements OnInit {
  ...
  loadCounterProducts() {
    this.pouchService.getCounterProducts().then(items => {
      items = items.filter(data => data.branch == 'Benin Centre');
